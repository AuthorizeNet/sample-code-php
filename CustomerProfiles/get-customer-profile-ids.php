<?php
 ...
 //  4. Validate Profiiel ID returned
  ...
  if (($response != null) && ($response->getMessages()->getResultCode() == "Ok") )
  {
      ...
      $selectedPaymentProfile = $response->getCustomerProfileId();
   }
  else
  {
      echo "ERROR :  CreateCustomerProfile: Invalid response\n";
  }
  // Now get an existing customer profile along with all the associated payment profiles and shipping addresses
  $request = new AnetAPI\GetCustomerProfileRequest();
  $request->setMerchantAuthentication($merchantAuthentication);
  $request->setCustomerProfileId( $selectedPaymentProfile );
